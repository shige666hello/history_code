...
def jobs(job_id):
    if request.method == "GET":
        if job_id is not None:
            ...
            for k in sorted(
                render_files,
                key=lambda x: (int(re.split(regex, x)[1]), int(re.split(regex, x)[2])) if len(re.split(regex, x)) > 2 else (float('inf'), float('inf'))
            ):
...
def download():
    ...
    logging.info(f"====>download: {directory}, {filename}")
